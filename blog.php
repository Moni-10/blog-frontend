...
$site = null;
foreach (($sitesData['websites'] ?? []) as $candidate) {
    if (($websiteId && ($candidate['_id'] ?? '') === $websiteId) || clean_domain($candidate['domain'] ?? '') === clean_domain($domain)) { $site = $candidate; $websiteId = $candidate['_id']; break; }
}
$blog = ($slug && $websiteId) ? api_get($apiBase . '/api/blogs/slug/' . rawurlencode($slug) . '?websiteId=' . rawurlencode($websiteId)) : null;
if (!$blog) {

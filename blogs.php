<?php
$apiBase = rtrim(getenv('BLOG_API_BASE_URL') ?: 'https://api.mmwmachine.com', '/');
...
$sitesData = api_get($apiBase . '/api/websites/all');
$matchedSite = null;
foreach (($sitesData['websites'] ?? []) as $site) {
  if (($websiteId && ($site['_id'] ?? '') === $websiteId) || clean_domain($site['domain'] ?? '') === clean_domain($domain)) { $matchedSite = $site; $websiteId = $site['_id']; break; }
}
$allBlogs = $websiteId ? api_get($apiBase . '/api/blogs/website/' . rawurlencode($websiteId)) : null;
$publishedBlogs = is_array($allBlogs) ? array_values(array_filter($allBlogs, fn($item) => ($item['status'] ?? 'published') === 'published')) : [];
$total = count($publishedBlogs); $limit = 9; $pages = max((int)ceil($total / $limit), 1);
$blogs = array_slice($publishedBlogs, ($page - 1) * $limit, $limit);
$data = is_array($allBlogs) ? true : null;
